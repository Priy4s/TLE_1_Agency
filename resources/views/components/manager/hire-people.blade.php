        ...
        <div class="flex justify-between gap-4 items-center mb-6">
            ...
            @if ($maxCandidates > 0)
                <div class="relative">
                    ...
                    <!-- Dropdown (Modal) Menu -->
                    <div id="hire-dropdown"
                        class="dropdown-content hidden fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
                        <div class="bg-white shadow-lg rounded-lg p-6 w-96">
                            <form action="{{ route('job.confirmHire', $job->id) }}" method="POST" class="space-y-4">
                                @csrf
                                <input type="hidden" name="job_id" value="{{ $job->id }}">
                                <label for="num_candidates" class="text-lg font-medium text-gray-800 block">Select
                                    Number of Candidates:</label>
                                <select name="num_candidates" id="num_candidates"
                        ...
                        @foreach ($waitlistUsers->where('status', 'hired') as $index => $waitlist)
                            <tr>
                                ...
                                <td class="border px-4 py-2 text-center">
                                    <a href="{{ route('chat.show', $waitlist->user->id) }}"
                                        class="bg-[#E2ECC8] hover:bg-[#D1E0A9] text-black font-bold py-2 px-4 rounded shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50 transition duration-300 inline-block">
                                        Message
                                    </a>
                                </td>
                                <td class="border px-4 py-2 text-center">
                                    <form action="{{ route('waitlist.updateProcess', $waitlist->id) }}" method="POST">
                                        ...
                                        <select name="process" onchange="this.form.submit()"
                                            class="w-48 p-2 border rounded-lg mt-4"> <!-- mt-2 toegevoegd -->
                                            <option value="Need to invite"
                                                {{ $waitlist->process === 'Need to invite' ? 'selected' : '' }}>Need to
                                                invite</option>
                                            <option value="Waiting for response"
                                                {{ $waitlist->process === 'Waiting for response' ? 'selected' : '' }}>
                                                Waiting for response</option>
                                            <option value="Done"
                                                {{ $waitlist->process === 'Done' ? 'selected' : '' }}>Done</option>
                                        </select>
                                    </form>
                                </td>
                            </tr>
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 w-96">
        ...
        <div class="flex justify-center gap-4">
            ...
            <button onclick="confirmHire()"
                class="bg-[#E2ECC8] hover:bg-[#D1E0A9] text-black py-2 px-4 rounded-lg">Confirm</button>
        </div>
    </div>
</div>
